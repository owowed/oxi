...
- In order to make release, select the current draft release document generated by release-drafter, and edit the document if needed.
    - **Please wait for the "Release Drafter" action to complete.** You can check this by seeing if there is green checkmark in the latest commit of `master`. Failure to do this will result in a commit tag to outdated `package` branch.
